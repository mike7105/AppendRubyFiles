...
def count_lines(filename, chunk_size=1 << 13) -> int:
    """считает кол-во строк в файле
    :param str filename: имя и путь к файлу
    :param int chunk_size: по crjkmrj cnhjr cxbnsdfnm
...
class Application(ttk.Frame):
    ...
    def create_menu(self):
        ...
        self.filemenu.add_command(label="Open settings", accelerator="Ctrl+O", command=self.open_settings)
        self.bind_all("<Control-KeyPress-o>", self.open_settings)

        self.filemenu.add_command(label="Save settings", accelerator="Ctrl+S", command=self.save_settings)
    ...
    def create_widgets(self):
        ...
        self.lbl_prev.grid(row=0, column=0, columnspan=2, sticky="we", **self.grid_params)
        ...
        self.ent_prev.bind("<Button-3>", lambda evt, obj=self.var_prev: self.show_menu(evt, obj))
        ...
        self.lbl_curr.grid(row=2, column=0, columnspan=2, sticky="we", **self.grid_params)
        ...
        self.ent_curr.bind("<Button-3>", lambda evt, obj=self.var_curr: self.show_menu(evt, obj))
        ...
        self.lbl_res.grid(row=4, column=0, columnspan=2, sticky="we", **self.grid_params)
        ...
        self.ent_res.bind("<Button-3>", lambda evt, obj=self.var_curr: self.show_menu(evt, obj))
    ...
    def run_append(self):
        ...
        if self.validate():
            ...
            if os.path.exists(os.path.join(self.var_prev.get(), "TOTAL.cd")):
                self.num_prev = count_lines(os.path.join(self.var_prev.get(), "TOTAL.cd"))
    ...
    def run_append_th(self):
        ...
        while not self.que.empty():
            ...
            if os.path.exists(local.cd_file_p):
                self.num_same += 1
                if self.num_prev == 0:
                    self.num_prev = count_lines(local.cd_file_p)
            ...
            # если файла нет, то создать пустой с определенным кол-вом строк
                ...
                self.num_new += 1
                local.cd_file_r = os.path.join(self.var_res.get(), os.path.split(local.cd_file_c)[1])
    ...
    def show_info(self):
        """Показ информации"""
        msgbox.showinfo("О программе...", f"""{self.version}\n© Михаил Чесноков, 2019 г.
mailto: Mihail.Chesnokov@ipsos.com""", parent=self)
    ...
    def open_settings(self, evt=None):
        ...
        filename = tkFD.askopenfilename(title="Open settings", filetypes=(("Settings", "JSON"),))
        if filename:
            ...
            msgbox.showerror(title="settings", message="File wasn't chosen!")

    def save_settings(self, evt=None):
        """Сохранение введенных параметров"""
        filename = tkFD.asksaveasfilename(title="Save settings", filetypes=(("Settings", "JSON"),))
        if filename:
            with open(filename + ".json", "w") as fln:
                obj = {'prev': self.var_prev.get(), 'curr': self.var_curr.get(), 'res': self.var_res.get()}
                json.dump(obj, fln)
        else:
...
def resource_path(relative_path) -> str:
    ...
    :param str relative_path: относительный путь к файлу
